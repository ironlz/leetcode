...
public class Init {
     ...
     * 著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
     */
    public int removeDuplicatesAdvance(int[] nums) {
        int currentIndex = 0;
        int index = 0;
        int repeatedCount = 0;
        while (index < nums.length) {
            if (index != currentIndex) {
                if (nums[index] == nums[currentIndex] && repeatedCount < 1) {
                    nums[++currentIndex] = nums[index];
                    repeatedCount++;
                } else if (nums[index] != nums[currentIndex]) {
                    nums[++currentIndex] = nums[index];
                    repeatedCount = 0;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        int element = init.removeDuplicatesAdvance(nums);
